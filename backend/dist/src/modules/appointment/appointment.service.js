...
const notifyUsers = async (_payload) => {
    try {
...
export const updateAppointmentStatus = async (appointmentId, status, userId, userRole) => {
    ...
    assertStatusTransition(appointment.status, status, userRole, appointment.appointmentDate);
    const now = new Date();
...
export const getBarberAvailability = async (salonId, dateString, requestedBarberId, serviceIds, clientId_for_overlap_check) => {
    ...
    const totalDurationMinutes = requestedServices.length
        ? requestedServices.reduce((sum, service) => sum + (service.durationMinutes || 30), 0)
        : 30;
    const slots = [];
    ...
    return slots.map((slot) => {
        ...
        const slotEnd = new Date(slotStart);
        slotEnd.setMinutes(slotEnd.getMinutes() + totalDurationMinutes);
        const isBarberBusy = appointments.some((appt) => slotStart < appt.estimatedEndTime && slotEnd > appt.appointmentDate);
        const isClientBusy = clientAppointments.some((appt) => slotStart < appt.estimatedEndTime && slotEnd > appt.appointmentDate);
        const isAvailable = !isBarberBusy && !isClientBusy;
        return { time: slot, available: isAvailable };
    });
};
export const getAvailableDatesForRange = async (salonId, startDateStr, endDateStr, requestedBarberId, serviceIds, clientId_for_overlap_check) => {
    const start = new Date(startDateStr);
    const end = new Date(endDateStr);
    // Limit to max 31 days to prevent overload
    if ((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24) > 31) {
        throw new Error("La plage de dates ne peut pas d�passer 31 jours.");
    }
    const dateStrings = [];
    ...
    const availabilityResults = await Promise.all(dateStrings.map(async (dateString) => {
        const slots = await getBarberAvailability(salonId, dateString, requestedBarberId, serviceIds, clientId_for_overlap_check);
        return {
            dateString,
...
export const createClientAppointment = async (clientId, salonId, barberId, dateString, timeString, serviceIds, targetType = 'EMPLOYEE') => {
    ...
    const totalDurationMinutes = services.reduce((sum, service) => sum + (service.durationMinutes || 30), 0);
    ...
    if (appointmentDate < openHour || estimatedEndTime > closeHour) {
        throw new Error('Le cr�neau d�passe les horaires du salon');
    }
    const availability = await getBarberAvailability(salonId, dateString, barberId, serviceIds);
    if (!availability.some((s) => s.time === timeString && s.available)) {
    ...
    const recipients = targetType === 'EMPLOYEE'
        ? [targetBarberId, salon.patronId].filter((id, idx, arr) => Boolean(id) && arr.indexOf(id) === idx)
        : [salon.patronId];
    await notifyUsers({
        title: 'Nouvelle demande de rendez-vous',
        body: `Service r�serv� � ${timeString}`,
        userIds: recipients,
        appointmentId: appointment.id,
        type: 'APPOINTMENT_UPDATED'
    });
    // Silent broadcast to ensure all apps refresh
    const allInvolved = [clientId, targetBarberId, salon.patronId].filter((id) => Boolean(id));
...
export const processInProgressReminders = async () => {
    ...
    for (const appointment of appointments) {
        await notifyUsers({
            title: 'Le client est-il arriv� ?',
            body: 'Confirmez son arriv�e pour d�marrer la prestation.',
...
export const processCompletionAlerts = async () => {
    ...
    for (const appointment of toPrompt) {
        ...
        await notifyUsers({
            title: 'Temps �coul�',
            body: 'As-tu termin� ? (Oui / Snooze +15min)',
            userIds: [appointment.barberId ?? appointment.salon.patronId],
            appointmentId: appointment.id
        });
    }
...
const emitStatusNotifications = async (appointmentId, clientId, barberId, patronId, status, appointmentDate) => {
    if (status === 'CONFIRMED') {
        await notifyUsers({
            title: 'Rendez-vous confirm�',
            body: `Votre rendez-vous a �t� accept� pour le ${appointmentDate.toLocaleDateString('fr-FR')} � ${appointmentDate.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}`,
            userIds: [clientId, barberId, patronId].filter((id) => Boolean(id)),
            appointmentId,
            type: 'APPOINTMENT_UPDATED'
        });
    }
    if (status === 'CANCELLED') {
        ...
        const recipients = potentialRecipients.filter((id, idx, arr) => Boolean(id) && arr.indexOf(id) === idx);
        await notifyUsers({
            title: 'Rendez-vous annul�',
            body: 'Une modification a �t� effectu�e sur le rendez-vous.',
            userIds: recipients,
            appointmentId,
            type: 'APPOINTMENT_UPDATED'
        });
    ...
    if (status === 'DECLINED') {
        await notifyUsers({
            ...
            body: 'Malheureusement, le salon a refus� ce cr�neau. Veuillez en choisir un autre.',
            userIds: [clientId, barberId, patronId].filter((id) => Boolean(id)),
            appointmentId,
            type: 'APPOINTMENT_UPDATED'
        });
    ...
    if (status === 'COMPLETED') {
        ...
        await notifyUsers({
            ...
            body: 'Votre prestation est termin�e, partagez votre review.',
            userIds: [clientId, barberId, patronId].filter((id) => Boolean(id)),
            appointmentId,
            type: 'APPOINTMENT_UPDATED'
        });
    ...
    const allInvolved = [clientId, barberId, patronId].filter((id) => Boolean(id));
    const uniqueInvolved = Array.from(new Set(allInvolved));
    const users = await prisma.user.findMany({
        where: { id: { in: uniqueInvolved } },
        include: { profile: true }
...
const assertStatusTransition = (currentStatus, nextStatus, userRole, appointmentDate) => {
    ...
    if (nextStatus === 'CANCELLED' && userRole === 'CLIENT') {
        ...
        if (new Date() >= oneHourBefore) {
            throw new Error('Annulation client impossible � moins de 1h du rendez-vous');
        }
    }
};
const assertUpdatePermission = async (appointment, userId, userRole) => {
...
const getSalonOpenCloseForDate = async (salonId, date) => {
    ...
    let closeTime = '17:00';
    if (workingHours?.isDayOff) {
        throw new Error('Salon ferm� ce jour');
    }
    if (workingHours?.openTime) {
        openTime = workingHours.openTime;
    }
...
export const getSalonAppointments = async (patronId) => {
...
export const getAppointmentsBySalonId = async (salonId) => {
    return prisma.appointment.findMany({
        where: { salonId },
        include: {
            client: { select: { fullName: true, phoneNumber: true, profile: { select: { avatarUrl: true } } } },
...
export const extendAppointment = async (appointmentId, minutes, userId, role) => {
    const appointment = await prisma.appointment.findUnique({
        where: { id: appointmentId },
        include: { salon: true }
    });
    ...
    if (role === 'EMPLOYEE' && appointment.barberId !== userId) {
        throw new Error("Non autoris� bch tzid wa9t");
    }
...
export const postponeNoShowWithCascade = async (appointmentId, minutes, userId, role) => {
    const appointment = await prisma.appointment.findUnique({
        where: { id: appointmentId },
        include: { salon: true }
    });
    ...
    const shiftedAppointments = await prisma.$transaction(async (tx) => {
    ...
    return {
        ...
        shiftedAppointmentIds: shiftedAppointments.map((appt) => appt.id)
    };
};
export const getUnreviewedAppointments = async (clientId) => {
    return prisma.appointment.findMany({
        where: {
            clientId,
            status: 'COMPLETED',
...
export const submitReview = async (appointmentId, clientId, salonId, rating, comment) => {
