...
const getSalonIdByPatronId = async (patronId) => {
...
export const createSalon = async (patronId, data) => {
    ...
    for (const admin of admins) {
        ...
        if (admin.profile?.fcmToken) {
            await sendNotification(admin.profile.fcmToken, "Nouveau salon", `Le salon "${newSalon.name}" est en attente d'approbation.`, { type: 'NEW_SALON', salonId: newSalon.id.toString() });
        }
    }
    return newSalon;
};
export const updateSalon = async (patronId, data) => {
    const existingSalon = await prisma.salon.findFirst({
        where: { patronId: patronId },
    });
    if (!existingSalon) {
...
export const getSalonByPatronId = async (patronId) => {
    const salon = await prisma.salon.findFirst({
        where: { patronId: patronId },
        include: {
            patron: {
...
export const createEmployeeAccount = async (patronId, data) => {
...
export const createEmployeeAccountAdmin = async (salonId, data) => {
...
export const updateEmployeeAccount = async (patronId, employeeId, data) => {
...
export const updateEmployeeAccountAdmin = async (salonId, employeeId, data) => {
    const employee = await prisma.user.findFirst({
        where: {
            id: employeeId,
            role: Role.EMPLOYEE,
...
export const removeEmployeeFromSalon = async (patronId, employeeId) => {
    const salonId = await getSalonIdByPatronId(patronId);
    const employee = await prisma.user.findFirst({
        where: {
            id: employeeId,
...
export const removeEmployeeFromSalonAdmin = async (salonId, employeeId) => {
    const employee = await prisma.user.findFirst({
        where: {
            id: employeeId,
            role: Role.EMPLOYEE,
...
export const createService = async (patronId, data) => {
    let salonIdToUse = data.salonId;
    if (!salonIdToUse) {
        const salon = await prisma.salon.findFirst({
            where: { patronId: patronId },
...
export const createServiceAdmin = async (salonId, data) => {
    const salon = await prisma.salon.findUnique({ where: { id: salonId } });
    if (!salon)
        throw new Error("Salon introuvable");
    const newService = await prisma.service.create({
...
export const getServices = async (patronId) => {
    const salon = await prisma.salon.findFirst({
        where: { patronId: patronId },
    });
    if (!salon) {
...
export const updateService = async (patronId, serviceId, data) => {
...
export const updateServiceAdmin = async (salonId, serviceId, data) => {
    const service = await prisma.service.findFirst({
        where: {
            id: serviceId,
            salonId: salonId
    ...
    if (!service)
        throw new Error("Service introuvable ou mouch mta3 l'salon hetha");
    const updatedService = await prisma.service.update({
        where: { id: serviceId },
        data: {
            ...(data.name !== undefined && { name: data.name }),
...
export const deleteService = async (patronId, serviceId) => {
    const service = await prisma.service.findFirst({
        where: {
            id: serviceId,
            salon: { patronId }
...
export const deleteServiceAdmin = async (salonId, serviceId) => {
    const service = await prisma.service.findFirst({
        where: {
            id: serviceId,
            salonId: salonId
...
export const getTopRatedSalons = async (limit = 10, includeUnapproved = false) => {
    const salons = await prisma.salon.findMany({
        where: !includeUnapproved ? { approvalStatus: 'APPROVED' } : {},
        orderBy: { rating: 'desc' },
        take: limit,
...
export const checkFavoriteStatus = async (clientId, salonId) => {
...
export const getFavoriteSalons = async (clientId) => {
    ...
    return favorites.map(fav => ({
        ...
        rating: fav.salon._count?.reviews > 0 && fav.salon.rating ? fav.salon.rating.toFixed(1) : "0.0",
    }));
};
export const createPortfolioImage = async (patronId, imageUrl) => {
    const salon = await prisma.salon.findFirst({
        where: { patronId }
    });
    if (!salon) {
...
export const deletePortfolioImage = async (patronId, imageId) => {
    const salon = await prisma.salon.findFirst({
        where: { patronId }
    });
    if (!salon) {
...
export const createPortfolioImageAdmin = async (salonId, imageUrl) => {
    return await prisma.portfolioImage.create({
        data: {
            salonId,
            imageUrl
...
export const deletePortfolioImageAdmin = async (salonId, imageId) => {
    return await prisma.portfolioImage.delete({
        where: {
            id: imageId,
            salonId
...
export const updateSalonStatusAdmin = async (salonId, status) => {
    return prisma.salon.update({
        where: { id: salonId },
        data: { approvalStatus: status }
    });
};
export const deleteSalonAdmin = async (salonId) => {
    return prisma.salon.delete({
        where: { id: salonId }
    });
};
export const updateSalonAdmin = async (salonId, data) => {
    const updatedSalon = await prisma.salon.update({
        where: { id: salonId },
        data: {
            ...(data.name !== undefined && { name: data.name }),
...
export const getSalonStatsAdmin = async (salonId) => {
