        ...
        try {
            ...
            for (const appt of appointments) {
                ...
                if (normalizedApptTime.getTime() === oneHourTenFromNow.getTime() && !appt.is1hReminderSent) {
                    if (clientToken) await sendNotification(clientToken, "Rappel de rendez-vous", `Votre rendez-vous est dans 1h10. Attention: l'annulation sera impossible si le temps restant est inférieur à 1 heure.`);
                    if (barberToken) await sendNotification(barberToken, "Prochain client", `Vous avez un rendez-vous dans 1h10 avec ${appt.client.fullName}.`);
                }
                ...
                if (normalizedApptTime.getTime() === oneHourFromNow.getTime()) {
                    ...
                    if (clientToken) await sendNotification(clientToken, "Rendez-vous dans 1h", "Votre rendez-vous est dans exactement 1 heure. L'annulation n'est plus possible maintenant.");
                    if (barberToken) await sendNotification(barberToken, "Prochain client", `Rappel: rendez-vous dans 1h avec ${appt.client.fullName}.`);
                }
