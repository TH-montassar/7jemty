...
export const initializeFirebase = () => {
    try {
        if (!admin.apps || admin.apps.length === 0) {
            ...
            if (!credential) {
                console.warn(
                    '[FCM] Firebase Admin credentials are missing. Push notifications are disabled until you configure FIREBASE_SERVICE_ACCOUNT_JSON or FIREBASE_PROJECT_ID/FIREBASE_CLIENT_EMAIL/FIREBASE_PRIVATE_KEY or GOOGLE_APPLICATION_CREDENTIALS.'
                );
                return;
            }
